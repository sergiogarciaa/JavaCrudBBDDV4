   ...
   String queryAll = "SELECT * FROM \"gbp_almacen\".\"gbp_alm_cat_libros\"";
    ...
    public ArrayList<LibroDto> select(Connection conexionGenerada, boolean tipoSelect) {
        ...
        try {
            ...
            if(tipoSelect){
                ...
                resultadoConsulta = declaracionSQL.executeQuery(queryAll);
            ...
            // Query de 1 en concreto
                ...
                queryParametizada = "SELECT * FROM gbp_almacen.gbp_alm_cat_libros WHERE id_libro = ? ";
                declaracionParametizada = conexionGenerada.prepareStatement(queryParametizada);
    ...
    public ArrayList<LibroDto> update(Connection conexionGenerada) {
        try {
            ...
            declaracionSQL = conexionGenerada.createStatement();
            // Query para mostrar la tabla entera
            resultadoConsulta = declaracionSQL.executeQuery(queryAll);
            //Llamada a la conversión a dto]
            listaLibros = adto.resultsALibrosDto(resultadoConsulta);
            ...
            if(libroEncontrado(conexionGenerada, libroElegido)){
                ...
                // Query parametizada
                queryParametizada = "UPDATE gbp_almacen.gbp_alm_cat_libros SET titulo = ?, autor = ?, isbn = ?, edicion = ? WHERE id_libro = ?";
            ...
            System.out.println("[ERROR-IMPLCrud-UpdateLibro] Error al actualizar el libro: " + e.getMessage());
        }
